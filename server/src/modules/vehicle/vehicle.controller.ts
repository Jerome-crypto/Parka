...
const vehicleSchema = z.object({
  plate: z.string().min(3).max(15).toUpperCase(),
  make: z.string().min(2),
  model: z.string().min(1),
  color: z.string().min(2),
  year: z.coerce.number().int().min(1900).max(new Date().getFullYear() + 1),
  type: z.enum(['sedan', 'suv', 'truck', 'motorcycle']),
});
...
export const createVehicle = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const vehicleExists = await query(
      'SELECT id, is_deleted FROM vehicles WHERE plate = $1',
      [validated.plate]
    );

    if (vehicleExists.rows.length > 0) {
      if (!vehicleExists.rows[0].is_deleted) {
        return next(new AppError('A vehicle with this plate is already registered.', 400));
      } else {
...
export const getVehicles = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const vehicles = await query(
      'SELECT id, plate, make, model, color, year, type FROM vehicles WHERE driver_id = $1 AND is_deleted = false ORDER BY created_at DESC',
      [req.user.id]
    );
...
export const updateVehicle = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const vehicle = await query(
      'SELECT id FROM vehicles WHERE id = $1 AND driver_id = $2 AND is_deleted = false',
      [id, req.user.id]
    );
...
export const deleteVehicle = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Soft delete
    await query(
      'UPDATE vehicles SET is_deleted = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
      [id]
    );
