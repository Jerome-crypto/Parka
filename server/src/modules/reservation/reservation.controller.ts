...
import { emitFacilityUpdate } from '../../services/socketService';

const reservationCreateSchema = z.object({
  facilityId: z.string().uuid(),
  vehicleId: z.string().uuid(),
  arrivalTime: z.string(), // ISO string or parsable date
  durationHours: z.coerce.number().int().positive().max(24),
});
...
export const createReservation = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (arrivalDate < new Date()) {
      return next(new AppError('Arrival time must be in the future.', 400));
    }
    ...
    const facilityRes = await client.query(
      `SELECT id, name, available_spaces, price_per_hour 
    ...
    if (facility.available_spaces <= 0) {
      await client.query('ROLLBACK');
      return next(new AppError('No spaces currently available in this facility.', 400));
    }

    // 2. Double-check and lock an available space row
    const spaceRes = await client.query(
      `SELECT s.id, s.space_number, z.name as zone_name
    ...
    if (spaceRes.rows.length === 0) {
      await client.query('ROLLBACK');
      return next(new AppError('No spaces currently available in the system.', 400));
    }
    ...
    const vehicleRes = await client.query(
      'SELECT id, plate FROM vehicles WHERE id = $1 AND driver_id = $2 AND is_deleted = false',
      [validated.vehicleId, req.user.id]
    );

    if (vehicleRes.rows.length === 0) {
      await client.query('ROLLBACK');
      return next(new AppError('Vehicle not found or not registered to you.', 404));
    }
    ...
    await client.query(
      ...
      [newAvailableSpaces, validated.facilityId]
    );
    ...
    const randSerial = crypto.randomBytes(2).toString('hex').toUpperCase();
    ...
    await client.query(
      `INSERT INTO reservation_statuses (reservation_id, status, notes)
       VALUES ($1, 'upcoming', 'Booking created.')`,
      [reservation.id]
    );
    ...
    emitFacilityUpdate(validated.facilityId, newAvailableSpaces);
...
export const getReservations = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.user.role === 'DRIVER') {
      ...
      const profileRes = await query('SELECT facility_id FROM attendant_profiles WHERE user_id = $1', [req.user.id]);
...
export const getReservationById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await query(
      `SELECT r.*, f.name as "facilityName", v.plate as "vehiclePlate", s.space_number as "spaceNumber", z.name as "zone"
       FROM reservations r
       JOIN parking_facilities f ON r.facility_id = f.id
       JOIN vehicles v ON r.vehicle_id = v.id
       LEFT JOIN parking_spaces s ON r.space_id = s.id
    ...
    if (req.user.role === 'DRIVER' && reservation.driver_id !== req.user.id) {
      return next(new AppError('Unauthorized access to this reservation.', 403));
    }
...
export const cancelReservation = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const resResult = await client.query(
      'SELECT id, driver_id, facility_id, space_id, status FROM reservations WHERE id = $1 FOR UPDATE',
      [id]
    );
    ...
    if (reservation.status !== 'upcoming') {
      await client.query('ROLLBACK');
      return next(new AppError(`Cannot cancel a reservation in '${reservation.status}' status.`, 400));
    }
    ...
    await client.query(
      'UPDATE parking_facilities SET available_spaces = $1 WHERE id = $2',
      [newAvailableSpaces, reservation.facility_id]
    );
    ...
    emitFacilityUpdate(reservation.facility_id, newAvailableSpaces);
