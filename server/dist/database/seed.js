...
const seedDatabase = async (force = false) => {
    logger_1.logger.info('Checking if database seeding is required...');
    ...
    try {
        if (!force) {
            ...
            if (count > 0) {
                logger_1.logger.info('Database already has users. Skipping auto-seeding.');
                return;
            }
        }
        logger_1.logger.info('Seeding database with test data...');
      ...
      CASCADE
    `);
        logger_1.logger.info('Cleaned existing database tables.');
        // 2. Seed Roles
    ...
    `);
        ...
        logger_1.logger.info('Seeded user roles.');
        // Hash passwords
    ...
    `, [roleMap['ADMIN'], passwordHash]);
        logger_1.logger.info('Seeded user accounts.');
        // 4. Seed Vehicles
    ...
    `, [driverId]);
        ...
        logger_1.logger.info('Seeded driver vehicles.');
        // 5. Seed Facilities
      ...
      `, [
        ...
        logger_1.logger.info('Seeded parking facilities.');
...
if (require.main === module) {
    ...
    (0, exports.seedDatabase)(force)
        .then(() => {
        logger_1.logger.info('Seeding process finished.');
        process.exit(0);
    })
        .catch((err) => {
        logger_1.logger.error('Seeding execution failed:', err);
        process.exit(1);
    });
}
